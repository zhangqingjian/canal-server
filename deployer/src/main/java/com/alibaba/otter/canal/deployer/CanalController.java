...
public class CanalController {
    ...
    private Long                                     cid;
    private String                                   ip;
    //手动添加的，因为当deployer构建成docker容器的时候，无法实现将指定外部访问的ip注册到zookeeper中
    private String                                   ipInZookeeper;
    private int                                      port;
    // 默认使用spring的方式载入
    ...
    public CanalController(final Properties properties){
        ...
        ipInZookeeper = getProperty(properties, CanalConstants.CANAL_IP_IN_ZOOKEEPER);
        ...
        //如果没有手动定义服务在zookeeper中显示的ipInZookeeper，则取原来的ip
        if(StringUtils.isEmpty(ipInZookeeper))
        {
            ipInZookeeper=ip;
        }

        final ServerRunningData serverData = new ServerRunningData(cid, ipInZookeeper + ":" + port);
...
//                                        ip + ":" + port);
    ...
    public void start() throws Throwable {
        ...
        logger.info("## start the canal server in zookeeper [{}:{}]", ipInZookeeper, port);
        // 创建整个canal的工作节点
        final String path = ZookeeperPathUtils.getCanalClusterNode(ipInZookeeper + ":" + port);
        initCid(path);
        if (zkclientx != null) {
    ...
    public void stop() throws Throwable {
        ...
        // 释放canal的工作节点
        releaseCid(ZookeeperPathUtils.getCanalClusterNode(ipInZookeeper + ":" + port));
        logger.info("## stop the canal server[{}:{}]", ip, port);
        logger.info("## stop the canal server in zookeeper [{}:{}]", ipInZookeeper, port);
